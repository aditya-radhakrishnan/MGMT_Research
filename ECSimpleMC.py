...
with graph.as_default():
    ...
    def stack_model(tf_stack_data, patch_size_c1, num_feat_c1):
        weights_c1, biases_c1 = weight_bias_variables([word_length, patch_size_c1, 1, num_feat_c1])
        first_conv_dim = (num_words - (patch_size_c1 - 1)) * num_feat_s1_c1

        print('tf_stack_data shape', tf_stack_data.get_shape())
        h_conv1 = tf.nn.relu(conv2d(tf_stack_data, weights_c1) + biases_c1)
        print('h_conv1 shape', h_conv1.get_shape())
        h_conv1_flat = tf.reshape(h_conv1, [-1, first_conv_dim])
        print('h_conv1_flat shape', h_conv1_flat.get_shape())
        weights_full_conn1, biases_full_conn1 = weight_bias_variables([first_conv_dim, num_full_conn_1])
        return tf.matmul(h_conv1_flat, weights_full_conn1) + biases_full_conn1
    '''
    flat_integ_dim = 3 * int(math.floor((num_words - (patch_size_s1_c1 - 1)) / pool_size_s1_c1)) * num_feat_s1_c1
    reshaped_size = flat_integ_dim
    print('reshaped size', reshaped_size)
    # reshaped_size = 3 * num_words * word_length
    ...
    weights_output, biases_output = weight_bias_variables([reshaped_size, num_labels])

    def stack_model(tf_stack_data, patch_size_c1, pool_size_c1, num_feat_c1):
        print('tf_stack_data shape', tf_stack_data.get_shape())
        weights_c1, biases_c1 = weight_bias_variables([word_length, patch_size_c1, 1, num_feat_c1])
        first_conv_dim = int(math.floor((num_words - (patch_size_c1 - 1)) / pool_size_c1))
        size = first_conv_dim * 1 * num_feat_c1
        print('size', size)
        # weights_full_conn1, biases_full_conn1 = weight_bias_variables([size, num_full_conn_1])
        h_conv1 = tf.nn.relu(conv2d(tf_stack_data, weights_c1) + biases_c1)
        h_pool1 = max_pool(h_conv1, pool_size_c1)
        print('h_conv1 shape', h_pool1.get_shape())
        return tf.reshape(h_pool1, [-1, size])

    '''
    def stack_model(tf_stack_data):
        print('tf_stack_data shape', tf_stack_data.get_shape())
        tf_reshaped_data = tf.reshape(tf_stack_data, [-1, num_words * word_length])
        weights_full_conn1, biases_full_conn1 = weight_bias_variables([num_words * word_length, num_full_conn_1])
        return tf.matmul(tf_reshaped_data, weights_full_conn1) + biases_full_conn1

    def model(tf_s1, tf_s2, tf_s3, keep_prob=tf.constant(1.0)):
        h_s1 = stack_model(tf_s1)
        h_s2 = stack_model(tf_s2)
        h_s3 = stack_model(tf_s3)
        h_full_conn_1 = tf.nn.relu(tf.concat(2, [h_s1, h_s2, h_s3]))
        print('h_full_conn_1 shape', h_full_conn_1.get_shape())
        h_drop1 = tf.nn.dropout(h_full_conn_1, keep_prob)
        h_flat_integ = tf.reshape(h_drop1, [-1, reshaped_size])
    ...
    '''
    def model(tf_s1, tf_s2, tf_s3, keep_prob=tf.constant(1.0)):
        h_s1 = stack_model(tf_s1, patch_size_s1_c1, pool_size_s1_c1, num_feat_s1_c1)
        h_s2 = stack_model(tf_s2, patch_size_s2_c1, pool_size_s2_c1, num_feat_s2_c1)
        h_s3 = stack_model(tf_s3, patch_size_s3_c1, pool_size_s3_c1, num_feat_s3_c1)
        print('h_s1 shape', h_s1.get_shape())
        h_full_conn_1 = tf.nn.relu(tf.concat(1, [h_s1, h_s2, h_s3]))
        print('h_full_conn_1 shape', h_full_conn_1.get_shape())
        h_drop1 = tf.nn.dropout(h_full_conn_1, keep_prob)
        h_flat_integ = tf.reshape(h_drop1, [-1, reshaped_size])
        return tf.matmul(h_flat_integ, weights_output) + biases_output

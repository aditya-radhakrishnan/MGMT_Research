...
with graph.as_default():
    ...
    '''
    weights_c1, biases_c1 = weight_bias_variables([word_length, patch_size_s1_c1, 1, num_feat_s1_c1])
    weights_c2, biases_c2 = weight_bias_variables([1, patch_size_s1_c2, num_feat_s1_c1, num_feat_s1_c2])
    weights_output, biases_output = weight_bias_variables([num_full_conn_1, num_labels])
    first_conv_dim = int(math.floor((num_words - (patch_size_s1_c1 - 1)) / pool_size_s1_c1))
    second_conv_dim = int(math.floor((first_conv_dim - (patch_size_s1_c2 - 1)) / pool_size_s1_c2))
    size = second_conv_dim * 1 * num_feat_s1_c2
    weights_full_conn1, biases_full_conn1 = weight_bias_variables([size, num_full_conn_1])


    def stack_model(tf_stack_data, pool_size_c1, pool_size_c2):
        ...
        h_full_conn1 = tf.nn.relu(tf.matmul(h_norm2_flat, weights_full_conn1) + biases_full_conn1)
        return tf.matmul(h_full_conn1, weights_output) + biases_output
    '''
    ...
    logits = stack_model(tf_stack_data=tf_train_s1, pool_size_c1=pool_size_s1_c1, pool_size_c2=pool_size_s1_c2)
    ...
    '''
    train_prediction = tf.nn.softmax(logits)
    test_prediction = tf.nn.softmax(stack_model(tf_test_s1, pool_size_s1_c1, pool_size_s1_c2))
    valid_prediction = tf.nn.softmax(stack_model(tf_valid_s1, pool_size_s1_c1, pool_size_s1_c2))

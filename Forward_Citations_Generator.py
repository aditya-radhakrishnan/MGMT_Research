...
class ForwardCitationsGenerator(object):

    # the name of the database to access
    db_name = ''

    # the name of the table where the backward citations are held
    table_name = ''
    ...
    def __init__(self, database_name, table_name, citation_column_name, new_table_name):
        ...
        self.citation_column_name = citation_column_name
    ...
    def generate_new_table(self):
        ...
        for patent_id in patent_ids:
            query = conn.execute("SELECT COUNT(*) FROM " + self.table_name + " WHERE " + self.citation_column_name + "=" + patent_id)
            num_citations = -1
            for value in query:

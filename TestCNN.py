...
def gen_examples(num_docs):
    ...
    examples = np.reshape(examples, (num_docs, -1))
    # print(examples.shape)
    return examples
...
with graph.as_default():
    ...
    def model(tf_data):
        tf_reshaped_data = tf.reshape(tf_data, [-1, doc_dims[0], doc_dims[1], 1])
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        if (step % 50 == 0):
            ...
            # print(predictions)
            # print('Accuracy: %.1f%%' % accuracy(predictions, batch_labels))

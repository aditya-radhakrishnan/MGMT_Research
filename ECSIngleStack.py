...
num_words = 3 * preprocessor.max_num_words


def combine_stacks(s1, s2, s3):
    return np.concatenate((s1, s2, s3), axis=2)
...
print('train s2 shape', train_s2.shape)
print('train s3 shape', train_s3.shape)
# (911, 20, 600, 1)
train_data = combine_stacks(train_s1, train_s2, train_s3)
...
test_data = combine_stacks(test_s1, test_s2, test_s3)
...
valid_data = combine_stacks(valid_s1, valid_s2, valid_s3)
...
with graph.as_default():
    ...
    weights_conv1 = weight_variable([word_length, patch_sizes1, 1, num_features_conv1])
    biases_conv1 = bias_variable([num_features_conv1])

    # Second set of weights
    weights_conv2 = weight_variable([1, patch_sizes2, num_features_conv1, num_features_conv2])
    biases_conv2 = bias_variable([num_features_conv2])
    ...
    weights_full_conn = weight_variable([size, num_full_conn])
    biases_full_conn = bias_variable([num_full_conn])

    # Weights and biases for softmax layer
    weights_output = weight_variable([num_full_conn, num_labels])
